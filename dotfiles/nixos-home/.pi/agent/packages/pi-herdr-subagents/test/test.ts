...
describe("session.ts", () => {
  ...
  describe("findLastAssistantMessage", () => {
    ...
    it("reports error and length stops when no errorMessage is available", () => {
      for (const stopReason of ["error", "length"]) {
        ...
        assert.equal(
          ...
          `Subagent error: stopReason=${stopReason} (no errorMessage field).`,
        );
      }
    });
  });
...
describe("subagent-done.ts", () => {
  ...
  describe("shouldAutoExitOnAgentEnd", () => {
    ...
    it("still exits when the latest turn ended with an error or length stop", () => {
      // Auto-exit subagents must shut down on terminal failures so the parent
      // receives the error sidecar instead of leaving the worker stranded.
      for (const stopReason of ["error", "length"]) {
        const messages = [{ role: "assistant", stopReason, errorMessage: "agent stopped" }];
        assert.equal(shouldAutoExitOnAgentEnd(false, messages), true);
      }
    });
  });
  ...
  describe("findLatestAssistantError", () => {
    ...
    it("falls back to a placeholder for error and length stops", () => {
      for (const stopReason of ["error", "length"] as const) {
        const messages = [{ role: "assistant", stopReason }];
        const info = findLatestAssistantError(messages);
        assert.ok(info);
        assert.equal(info!.stopReason, stopReason);
        assert.match(info!.errorMessage, new RegExp(`stopReason=${stopReason}`));
      }
    });
  ...
  describe("buildCompletionSidecar", () => {
    ...
    it("preserves error and length stops in the immediate completion sidecar", () => {
      for (const stopReason of ["error", "length"] as const) {
        assert.deepEqual(buildCompletionSidecar([
          { role: "assistant", stopReason, errorMessage: "agent failed" },
        ]), {
          type: "error",
...
describe("completion.ts", () => {
  ...
  it("decodes error and length payloads with a non-zero exit code", () => {
    for (const stopReason of ["error", "length"] as const) {
      assert.deepEqual(
...
describe("subagent interruption", () => {
  ...
  it("returns an explicit error without aborting when pane close fails", () => {
    const testApi = (subagentsModule as any).__test__;
    let aborted = false;
    ...
    assert.match(result.error, /Failed to terminate/);
    assert.equal(aborted, false);
  });

  it("leaves status unchanged when pane close fails in the tool path", () => {
    const testApi = (subagentsModule as any).__test__;
    const runningMap = testApi.runningSubagents as Map<string, any>;
    runningMap.clear();
  ...
  it("closes the pane and aborts the watcher", () => {
    const testApi = (subagentsModule as any).__test__;
    let aborted = false;
    let closedSurface = "";
    const running = makeRunning({
  ...
  it("treats a missing pane as successful termination", () => {
    const testApi = (subagentsModule as any).__test__;
    const runningMap = testApi.runningSubagents as Map<string, any>;
    runningMap.clear();
  ...
  it("acknowledges termination and removes running state", () => {
    const testApi = (subagentsModule as any).__test__;
    const runningMap = testApi.runningSubagents as Map<string, any>;
    let sentSurface = "";
    runningMap.clear();
    ...
    try {
      ...
      assert.equal(result.content[0].text, 'Subagent "Worker" terminated.');
      assert.deepEqual(result.details, { id: "a1", name: "Worker", status: "terminated" });
      assert.equal(runningMap.has("a1"), false);
    } finally {
      runningMap.clear();
    }
  ...
  it("reports no running subagent after repeated termination", () => {
    const testApi = (subagentsModule as any).__test__;
    const runningMap = testApi.runningSubagents as Map<string, any>;
    ...
    try {
      ...
      const repeated = testApi.handleSubagentInterrupt({ name: "Worker" }, (surface: string) => {
        surfaces.push(surface);
      });

      assert.deepEqual(surfaces, ["pane-1"]);
      assert.match(repeated.content[0].text, /No running subagent/);
      assert.equal(runningMap.has("a1"), false);
    } finally {
      runningMap.clear();
    }
