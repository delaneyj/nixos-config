...
export function shouldAutoExitOnAgentEnd(
  ...
  //
  // stopReason: "error" or "length" (e.g. exhausted retries on a provider
  // overload) also returns true — we want to shut down so the parent is
  // woken up — but we pair this with findLatestAssistantError() so the parent
...
/**
 * If the latest assistant message in the turn ended with
 * `stopReason: "error"` or `stopReason: "length"`
 * (typically auto-retry exhausted on an overload / rate limit / server
 * error, or context length reached), return its stop info so the parent
 * orchestrator can surface a clear failure instead of silently treating the
 * run as completed.
 *
...
export function findLatestAssistantError(
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (stopReason !== "error" && stopReason !== "length") return null;
    const raw = typeof msg.errorMessage === "string" ? msg.errorMessage.trim() : "";
    return {
      errorMessage:
        raw || `Subagent agent loop ended with stopReason=${stopReason} (no errorMessage field).`,
      stopReason,
    };
  }
  return null;
...
export function buildCompletionSidecar(messages: any[] | undefined):
  | { type: "done" }
  | { type: "error"; errorMessage: string; stopReason: "error" | "length" } {
