 ...
 *
 * Falls back to a synthetic error summary when the latest assistant message
 * has `stopReason: "error"` or `stopReason: "length"` and no usable text.
 * This avoids returning a stale earlier summary when provider failure or
 * context-length exhaustion occurred after successful output.
 */
...
export function findLastAssistantMessage(entries: SessionEntry[]): string | null {
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (stopReason === "error" || stopReason === "length") {
      ...
      return `Subagent error: stopReason=${stopReason} (no errorMessage field).`;
    }
  }
  return null;
}
