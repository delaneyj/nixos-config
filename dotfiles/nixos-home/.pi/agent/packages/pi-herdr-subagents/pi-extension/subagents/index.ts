...
const SubagentParams = Type.Object({
  ...
  cwd: Type.Optional(
    Type.String({
      description:
        "Working directory for the sub-agent. The agent starts in this folder and picks up its local .pi/ config, context files, skills, and extensions. Use for role-specific subfolders.",
    }),
  ),
  fork: Type.Optional(
    Type.Boolean({
...
function isMissingSubagentPaneError(error: unknown): boolean {
  const sources = [
    error?.message,
    error?.stderr,
    error?.stdout,
  ...
  return sources.some((value) =>
    typeof value === "string" && /\b(?:pane_not_found|not_found|pane not found)\b/i.test(value),
  );
}
...
function requestSubagentInterrupt(
  ...
  try {
    try {
      ...
      if (!isMissingSubagentPaneError(error)) {
        throw error;
      }
    }

    running.abortController?.abort();
    running.lifecycle = markDelivery(ensureLifecycle(running), "suppressed");
    ...
    return {
      error:
        `Failed to terminate subagent "${running.name}" via herdr: ` +
...
function handleSubagentInterrupt(
  ...
  const interruption = requestSubagentInterrupt(running, closePaneKey);
  ...
  return {
    content: [{ type: "text" as const, text: `Subagent "${running.name}" terminated.` }],
    details: { id: running.id, name: running.name, status: "terminated" },
  };
}
